...
export function getHelpListPerformance ( ) : JSX.Element {
  ...
  const WebPartHelpElement = <PivotItem headerText={ 'Performance' } > 
        <div className={ 'fps-pph-content' }>
          ...
          <div>{`You may need to toggle the 'Get all item props' if a column is not a refiner or on a view.`}</div>
          <div>Having both rest and javascript eval filters allow you to limit what items and refiners you see.</div>
          <div>In Javascript eval filters, <b>{`everything is case sensitive`}</b> and requires using { LinkFindInternalName }</div>
          <ul>
            ...
            <li>{`The javascript syntax for an item's User columns is InternalColumName followed by either Id or Title - with no space or .dot.`}</li>
          </ul>
